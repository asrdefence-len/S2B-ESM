...
class PolarEmitterCanvas(FigureCanvas):
    ...
    def _configure_axes(self):
        ...
        for bearing_deg in range(0, 360, 45):
            ...
            ax.plot([angle, angle], [0.0, 1.0], color="black", linewidth=0.7,
                    alpha=0.16, zorder=0)
        ...
        ax.legend(handles=legend_items, title="ASSESSMENT", loc="upper right",
                  bbox_to_anchor=(-0.18, 1.08), framealpha=0.92, fontsize=8,
    ...
    def update_emitters(self, emitters, selected_index=0):
        ...
        for index, emitter in enumerate(emitters):
            ...
            ax.plot([angle, angle], [0.0, radius], color="black",
                    linewidth=1.5 if index == selected_index else 1.0,
                    alpha=0.75 if index == selected_index else 0.55, zorder=1)
            ...
            ax.text(angle, min(radius + 0.09, 0.98), label,
...
class S2BOperatorWindow(QMainWindow):
    def __init__(self):
        ...
        self.nav_beam = RotatingSincBeam(
            ...
            initial_azimuth_deg=45.0, sidelobe_floor_db=-50.0,
        )
        self.nav_tracker = EmitterIlluminationTracker(
            history_s=30.0, illumination_threshold_db=-18.0,
            persistent_s=0.75, peak_separation_s=0.5,
            baseline_confidence_threshold=0.50,
            change_confidence_threshold=0.50,
            change_hold_s=3.0,
        )
        self.nav_assessment = self.nav_tracker.assess(0.0)
        self.nav_level_dbfs = -55.0
    ...
    def _process_snapshot(self):
        ...
        source = SimulatedSource(sample_rate_hz=SAMPLE_RATE_HZ,
            center_frequency_hz=CENTER_FREQUENCY_HZ,
            emitters=scenario.emitters, noise_std=scenario.noise_std)
        iq, metadata = source.read()
        detector = PulseDetector(threshold=DETECTION_THRESHOLD,
            sample_rate_hz=metadata["sample_rate_hz"],
            min_pulse_width_s=MIN_PULSE_WIDTH_S)
        if self.extractor is None:
            self.extractor = PDWExtractor(sample_rate_hz=metadata["sample_rate_hz"],
        ...
        physical_hypotheses = PhysicalEmitterCorrelator(
            ...
            amplitude_scale_db=ASSOCIATION_AMPLITUDE_TOLERANCE_DB).correlate(hypotheses[0], [])
        ...
        for emitter_index, group in enumerate(groups, start=1):
            ...
            emitters.append({"emitter_id": emitter_id, "aoa_deg": mean_aoa,
                "state": state, "display_color": ASSESSMENT_COLORS[state],
                "watched": emitter_id in self.watched_emitters,
                "tracks": tracks, "current": current, "links": group["links"],
                "track_confidence": track_conf, "illumination": None})
    ...
    def _update_nav_radar(self):
        ...
        while t <= now_s + 1e-9:
            ...
            self.nav_level_dbfs = max(-55.0, -6.0 + gain_db) + random.gauss(0.0, 0.6)
            self.nav_assessment = self.nav_tracker.update(t, self.nav_level_dbfs)
            t += 0.01
        self.nav_last_sample_s = max(self.nav_last_sample_s, t)

    def _nav_emitter_record(self):
        a = self.nav_assessment
        state = a.system_assessment
        emitter_id = "E3"
        ...
        return {"emitter_id": emitter_id, "aoa_deg": 135.0, "state": state,
                ...
                "watched": emitter_id in self.watched_emitters, "tracks": [current],
                "current": current, "links": [], "track_confidence": max(0.0, a.confidence),
                "illumination": a}

    def _refresh(self):
        try:
            scenario, emitters, pulse_count = self._process_snapshot()
            self._update_nav_radar()
            emitters.append(self._nav_emitter_record())
            ...
            self.statusBar().showMessage(
                f"Scenario: {scenario.name} | IQ pulses: {pulse_count} | Physical emitters: {len(emitters)} | E3: 9.410 GHz nav radar")
    ...
    def _populate_table(self):
        ...
        for row, emitter in enumerate(self.emitters):
            ...
            values = (emitter["emitter_id"], f"{emitter['aoa_deg']:.1f} deg",
                f"{current['frequency_hz'] / 1e6:.3f}", current["modulation"],
                emitter["state"], "WATCH" if emitter.get("watched", False) else "",
    ...
    def _show_selected_emitter(self):
        ...
        self.emitter_heading.setText(f"{emitter['emitter_id']}  |  BEARING {emitter['aoa_deg']:.1f} deg{watch_text}")
        ...
        if illumination is not None:
            ...
            for track in emitter["tracks"]:
                ...
                lines.append(f"T{track['track_id']}: RF={track['frequency_hz']/1e6:.3f} MHz, PRI={tpri}, PW={track['pulse_width_s']*1e6:.3f} us, MOD={track['modulation']}")
        else:
            lines.append("No significant linked-track behaviour change currently detected.")
        lines.extend(["", "S2B INTERPRETATION", "------------------",
