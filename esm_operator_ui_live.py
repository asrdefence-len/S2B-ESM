...
class LiveS2BOperatorWindow(EnhancedS2BOperatorWindow):
    ...
    def _install_pdw_rate_plot(self):
        ...
        polar = self.polar
        parent = polar.parentWidget()
        layout = parent.layout()
        index = layout.indexOf(polar)
        layout.removeWidget(polar)
        row = QWidget(parent)
        row_layout = QHBoxLayout(row)
        row_layout.setContentsMargins(0, 0, 0, 0)
        row_layout.setSpacing(8)
        polar.setParent(row)
        self.pdw_rate_plot = PDWRateHistogramCanvas(row)
        row_layout.addWidget(polar, stretch=3)
        row_layout.addWidget(self.pdw_rate_plot, stretch=2)
        layout.insertWidget(index, row, stretch=1)

    def _new_stream(self):
        self._shutdown_engine()
        self.processing_engine = OperatorProcessingEngine(
            sample_rate_hz=40_000_000, center_frequency_hz=9_415_000_000,
            block_samples=self.BLOCK_SAMPLES, noise_std=0.02, workers=self.WORKERS,
        ...
        self.snapshot_tracks = {}; self.latest_processing_status = {}; self.last_stream_time_s = 0.0
    ...
    def _plot_track(snapshot):
        ...
        return SimpleNamespace(emitter_id=snapshot["emitter_id"],pdws=pdws,pri_state_history=snapshot.get("pri_state_history",[]))

    def _selected_track(self):
        if not self.emitters:return None
    ...
    def _update_pdw_rate_plot(self):
        if not hasattr(self,"pdw_rate_plot"):return
        if not self.emitters:
            self.pdw_rate_plot.clear_plot(); return
        eid=self.emitters[self.selected_emitter_index]["emitter_id"]
        snapshot=self.snapshot_tracks.get(eid)
        if snapshot is None:
            self.pdw_rate_plot.clear_plot(f"{eid}: WAITING FOR PDWs"); return
        self.pdw_rate_plot.update_history(eid,snapshot.get("pdw_rate_history",[]))

    def _open_measurement_plot(self):
        if self.measurement_plot is None:self.measurement_plot=EmitterMeasurementScatterWindow(self,max_points=4000)
    ...
    def start_system(self):
        ...
        self.processing_engine.start(); self.running=True; self._set_status("RUNNING"); self.timer.start(); self._refresh()
    ...
    def stop_system(self):
        ...
        self.running=False; self.timer.stop(); self.measurement_plot_timer.stop()
        if self.processing_engine is not None:self.processing_engine.stop()
        self._set_status("STOPPED"); status=self.latest_processing_status
    ...
    def reset_system(self):
        ...
        self.mode_history.clear(); self.library_memory.clear(); self.library_degraded.clear(); self.library_operator_confirmed.clear(); self.watched_emitters.clear(); self.operator_assessments.clear(); self._details_emitter_id=None
        self._new_stream(); self._show_prestart_blank(); self._set_status("STOPPED"); self._update_measurement_plot(); self._update_pdw_rate_plot()

    def _refresh(self):
        if not self.running or self.processing_engine is None:return
        try:
            snapshot=self.processing_engine.latest_snapshot()
            if snapshot is None:
                self.statusBar().showMessage("PROCESSING ENGINE | waiting for first state snapshot"); return
            status=snapshot.get("status",{}); self.latest_processing_status=status; now_s=float(status.get("completed_time_s",self.last_stream_time_s)); tracks=snapshot.get("tracks",[]); self.snapshot_tracks={t["emitter_id"]:t for t in tracks}
            out=[]
            for track in tracks:
                current=track["current"]; illum=IlluminationAssessment(**track["illumination"])
                ...
                e={"emitter_id":eid,"aoa_deg":self._display_aoa_snapshot(track),"system_state":system_state,"state":system_state,"display_color":self._assessment_color(system_state),"watched":eid in self.watched_emitters,"tracks":[],"current":current,"links":[],"track_confidence":min(1.0,track["total_pulses"]/20.0),"illumination":illum,"last_seen_s":track["last_seen_s"]}
