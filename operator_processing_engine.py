...
Operator controls remain simple: the GUI starts this engine from the same Python
invocation and exchanges START/STOP/RESET/SHUTDOWN commands plus ~10 Hz snapshots.
"""
...
def _feed_behaviour(behaviour, track, new_pdws, start_s, end_s, frequency_gate_hz):
    """Feed one emitter's measured amplitudes into 10 ms illumination bins."""
    relevant = [
        p for p in new_pdws
        if abs(p.frequency_hz - track.frequency_hz) <= frequency_gate_hz
    ]
    bin_s = 0.010
    ...
    b0 = int(start / bin_s)
    b1 = int(__import__("math").ceil(end / bin_s))
...
def _track_snapshot(track, illumination):
    # Keep only low-rate data needed by the UI.  Never send IQ or waveform
    # snippets across this boundary.
...
def _publish_latest(snapshot_q, snapshot):
    """Non-blocking latest-value queue: stale GUI snapshots are disposable."""
    try:
...
def _engine_main(command_q, snapshot_q, config):
    ...
    try:
        alive = True
        while alive:
            # Drain all currently queued operator commands so RESET/SHUTDOWN are
            # never delayed behind the 10 Hz snapshot cadence.
            while True:
                try:
            ...
            if running:
                new_pdws = processor.drain_pdws()
                status = processor.status()
                if new_pdws:
                    tracker.update(new_pdws)

                now_s = float(status["completed_time_s"])
                if now_s > last_completed_s:
                    for track in tracker.tracks:
                        latest_illumination[track.emitter_id] = _feed_behaviour(
                            behaviour,
                            track,
                            new_pdws,
                            last_completed_s,
                            now_s,
                            config["frequency_gate_hz"],
                        )
                ...
                if wall_now - last_publish_wall >= SNAPSHOT_PERIOD_S:
                    ...
                    for track in tracker.tracks:
                        ...
                        if illumination is None:
                            illumination = behaviour.assessment(track.emitter_id, now_s)
                    ...
                    last_publish_wall = wall_now
            else:
                time.sleep(0.02)

            if running:
                # The actual RF flow is driven by the producer thread.  A short
                # sleep keeps this orchestration loop from burning an extra core.
                time.sleep(0.005)
    finally:
        try:
            processor.stop()
...
class OperatorProcessingEngine:
    """Small GUI-side controller for the dedicated processing process."""
    ...
    def __init__(
        ...
        self._command_q = self._ctx.Queue(maxsize=16)
        self._snapshot_q = self._ctx.Queue(maxsize=3)
        self._config = {
            ...
            "classifier_queue_depth": int(classifier_queue_depth),
            "result_queue_depth": int(result_queue_depth),
            "frequency_gate_hz": float(frequency_gate_hz),
        }
        self._process = None
    ...
    def _ensure_process(self):
        ...
        self._process = self._ctx.Process(
            target=_engine_main,
            args=(self._command_q, self._snapshot_q, self._config),
            name="S2B-Processing-Engine",
        )
        self._process.start()
    ...
    def _send(self, command):
        ...
        try:
            self._command_q.put(command, timeout=0.5)
        except queue.Full:
            raise RuntimeError(f"S2B processing command queue full while sending {command}")
    ...
    def latest_snapshot(self):
        ...
        while True:
            try:
                latest = self._snapshot_q.get_nowait()
            except queue.Empty:
                break
        return latest
    ...
    def shutdown(self, timeout_s=4.0):
        ...
        if self._process.is_alive():
            try:
                self._command_q.put("SHUTDOWN", timeout=0.5)
            except queue.Full:
                pass
