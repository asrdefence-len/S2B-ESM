...
from scenario_runner import parse_args, select_scenario
...
def print_groups(title, groups, analyzer):
    ...
    for group in groups:
        ...
        if "mean_pulse_width_s" in group:
            ...
            extra = (f", mean PW={mean_pw_us:.3f} us"
                     f", mean AMP={mean_amp_dbfs:.2f} dBFS"
                     f", modulation={modulation}"
                     f", estimated PRI={pri_text}")
        print(f"Candidate {candidate_id}: {len(group['pdws'])} pulses, "
              f"mean frequency={mean_frequency_mhz:.3f} MHz{extra}")
        for item in sequence:
            pdw = item["pdw"]
            pri_s = item["pri_s"]
            pri_text = "--------" if pri_s is None else f"{pri_s * 1e6:8.3f} us"
            print(f"  PDW {pdw.pdw_id:06d}  PW={pdw.pulse_width_s * 1e6:6.3f} us  "
                  f"AMP={pdw.amplitude_dbfs:6.2f} dBFS  MOD={pdw.modulation_type:3s}  "
                  f"BW={pdw.modulation_bandwidth_hz / 1e6:5.3f} MHz  PRI={pri_text}")
        print()
...
def print_hypotheses(hypotheses):
    ...
    for rank, hypothesis in enumerate(hypotheses[:MHT_DISPLAY_HYPOTHESES], start=1):
        ...
        print(f"Hypothesis {rank}: posterior={100.0 * hypothesis['probability']:.2f}%  "
        ...
        for candidate in hypothesis["candidates"]:
            ...
            print(f"  Track {candidate['candidate_id']}: {len(candidate['pdws'])} pulses, "
        ...
        if clutter_count:
            print("  Clutter PDWs: " + ",".join(str(p.pdw_id) for p in hypothesis["clutter_pdws"]))
        print()
...
def print_association_uncertainty(pdws, marginals, track_membership, best_hypothesis):
    ...
    for pdw in pdws:
        ...
        if candidate_id is None:
            ...
            print(f"PDW {pdw.pdw_id:06d}  CLUTTER  posterior={100.0 * other_weight:5.1f}%")
            continue
        family_weight = marginals[pdw.pdw_id].get(candidate_id, 0.0)
        track_weight = track_membership[pdw.pdw_id].get(candidate_id, 0.0)
        confidence = "HIGH" if track_weight >= 0.95 else ("MED " if track_weight >= 0.75 else "LOW ")
        print(f"PDW {pdw.pdw_id:06d}  T{candidate_id}  family={100.0 * family_weight:5.1f}%  "
              f"track={100.0 * track_weight:5.1f}%  track confidence={confidence}")
    print()


def make_mht(engine):
    cls = (GatedFastProbabilisticMultipleHypothesisAssociator
           if engine == "gated" else ProbabilisticMultipleHypothesisAssociator)
    return cls(
...
def main():
    ...
    extractor = PDWExtractor(sample_rate_hz=metadata["sample_rate_hz"],
    ...
    changes = PDWChangeDetector(
        ...
        bandwidth_scale_hz=MODULATION_LFM_BANDWIDTH_THRESHOLD_HZ).detect(pdws)
    frequency_groups = FrequencyAssociator(
        frequency_tolerance_hz=ASSOCIATION_FREQUENCY_TOLERANCE_HZ).associate(pdws)
    evidence_groups = EvidenceAssociator(
        ...
        amplitude_tolerance_db=ASSOCIATION_AMPLITUDE_TOLERANCE_DB).associate(pdws)

    mht = make_mht(args.mht)
    ...
    physical_hypotheses = PhysicalEmitterCorrelator(
        frequency_scale_hz=ASSOCIATION_FREQUENCY_TOLERANCE_HZ,
        amplitude_scale_db=ASSOCIATION_AMPLITUDE_TOLERANCE_DB).correlate(hypotheses[0], changes)
    operator_summaries = OperatorEmitterSummary().build(hypotheses, marginals, track_membership)
    truth_score = SimulationTruthScorer(sample_rate_hz=metadata["sample_rate_hz"]).score(
        scenario, pdws, hypotheses)
    ...
    print_groups("Greedy evidence groups - frequency + PW + amplitude + modulation + PRI",
                 evidence_groups, analyzer)
